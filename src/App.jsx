...
function generateStoryCard(entry) {
  ...
  grad.addColorStop(0,"#00007f"); grad.addColorStop(0.5,"#0000cd"); grad.addColorStop(1,"#1084d0");
  ctx.fillStyle = grad; ctx.fillRect(52,52,984,150);

  ctx.fillStyle = "#ffffff"; ctx.font = "bold 56px Arial, sans-serif";
  ctx.fillText("THE PUBLIC JOURNAL", 80, 112);

  const author = entry.anon ? "Anonymous" : entry.handle;
  const date = new Date(entry.ts).toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"});
  ctx.fillStyle = "#aad4ff"; ctx.font = "34px Arial, sans-serif";
  ctx.fillText(author + "  •  " + date, 80, 155);

  const rb = ["#f00","#f80","#ff0","#0c0","#00f","#80c","#f00"];
  const rw = 984/rb.length;
  rb.forEach((c,i)=>{ ctx.fillStyle=c; ctx.fillRect(52+i*rw,202,rw+2,18); });
  ...
  const bodyTop = 240; const bodyBottom = 1720;
  const textBlockH = lines.length * lh;
  const startY = bodyTop + (bodyBottom - bodyTop - textBlockH) / 2;
  ...
  ctx.fillStyle = "#000080"; ctx.font = "bold 180px Times New Roman, serif";
  ctx.textAlign = "right";
  ctx.fillText("”", 1010, y + 80);
  ctx.textAlign = "left";

  rb.forEach((c,i)=>{ ctx.fillStyle=c; ctx.fillRect(52+i*rw,1740,rw+2,18); });
  ctx.fillStyle = "#000080"; ctx.fillRect(52,1758,984,124);
  ctx.fillStyle = "#ffff00"; ctx.fillRect(52,1755,984,4);
  ctx.fillStyle = "#ffffff"; ctx.font = "bold 46px Arial, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("THEPUBLICJOURNAL.ONLINE", 540, 1828);
  ctx.fillStyle = "#aad4ff"; ctx.font = "30px Arial, sans-serif";
  ctx.fillText("Leave your story at thepublicjournal.online", 540, 1868);
  ctx.textAlign = "left";
  ...
  return (
    <div style={{ ...W.win, marginBottom:"12px" }}>
      ...
      <div style={{ background:"#d4d0c8", borderTop:"1px solid #808080", padding:"6px 10px", display:"flex", gap:"6px", alignItems:"center", flexWrap:"wrap" }}>
        ...
        <button style={{ ...W.btnSm, minHeight:"36px" }} onClick={async e=>{e.stopPropagation();const img=generateStoryCard(s);const b=await fetch(img).then(r=>r.blob());const u=URL.createObjectURL(b);window.open(u,'_blank');}}>📸 Story</button>
        <button style={{ ...W.btnSm, minHeight:"36px" }} onClick={async e=>{e.stopPropagation();const img=generateSquareCard(s);const b=await fetch(img).then(r=>r.blob());const u=URL.createObjectURL(b);window.open(u,'_blank');}}>🖼️ Post</button>
